    ...
    $.get("/getimg", function (res) {
        laypage.render({
            ...
            jump: function (obj, first) {
                $("#img-path").empty();
                $.get("/getimg", {"page": obj.curr, "limit": obj.limit}, function (data) {
                    if (data.code === 0) {
                        IMG_list = data.data;
                        for (let i = 0; i < IMG_list.length; i++) {
                            $("#img-path").append("<li><a href=" + IMG_list[i] + " target='_blank'><img class='layui-anim lazy' data-anim=layui-anim-scale' src=" + IMG_list[i] + "/></a></li>")
                        }
                    }
                }, "json");
                if (!first) {
